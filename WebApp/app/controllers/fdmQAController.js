...
app.controller('fdmQAController', ['$http', '$scope', '$routeParams', '$rootScope', 'fdmService', 'authService', 'notificationService', function ($http, $scope, $routeParams, $rootScope, fdmService, authService) {
    ...
    function updateFDMStatus(v) {
        console.log($scope.ConfirmationStatus);
        if ($scope.ConfirmationStatus == 0 || $scope.ConfirmationStatus == null) {
            fdmService.FDMStatus($scope.rowId, v, $rootScope.userName).then(function (resposnse) {
    ...
    $scope.btn_confirm = {
        ...
        onClick: function () {
            if ($scope.selectedTabId == "boeing") {
                fdmService.lockBoeingFDM($scope.year, $scope.month + 1, $scope.userName).then(function (response) {
                ...
                fdmService.lockMDFDM($scope.year, $scope.month + 1, $scope.userName).then(function (response) {
